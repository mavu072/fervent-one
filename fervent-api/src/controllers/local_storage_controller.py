...
def upload_file(filename: str, file: BinaryIO | TextIO, size: int, mime_type: str):
    ...
    try:
        validate_uploaded_file(
            filename=filename, file=file, size=size, mime_type=mime_type
        )
        ...
        if mime_type == "text/plain":
            ...
            write_to_file(
                filepath=filepath, content=contents
            )  # Read and save text file.
            page_no = 1  # Default to 1 page for text files.
        else:
            images = pdf_to_image(
                file=file, mime_type=mime_type
            )  # Convert PDF and images to Image objects.
